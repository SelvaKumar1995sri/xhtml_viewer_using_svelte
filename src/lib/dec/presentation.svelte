<script lang="ts">
    ...
    console.log('📖 Presentation component mounted with book:', book);

	onMount(async () => {
		if (book) {
            try {
                console.log('📥 Fetching slides for book:', book); // Debugging: Log fetch start
                const res = await fetch(`/api/html_files?book=${book}`);
                console.log(`/api/html_files?book=${book}`);
                if (!res.ok) {
                    throw new Error(`Failed to fetch slides for book: ${book}`);
                }
                slideContents = await res.json(); // Already cleaned XHTML content
                console.log('✅ Loaded cleaned slide contents:', slideContents); // Debugging: Log fetched slides
            } catch (err) {
                console.error('❌ Error loading slides:', err); // Debugging: Log fetch error
            }
        } else {
            console.warn('⚠️ No book provided to load slides.'); // Debugging: Warn if no book is provided
        }
	});
